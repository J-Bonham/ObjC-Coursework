...
-(void)didMoveToView:(SKView *)view {
    ...
    _tableTitle.tintColor = [SKColor greenColor];
    ...
    NSData *data = [[NSUserDefaults standardUserDefaults] objectForKey:@"leaderboard2"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //cell.textLabel.text = [NSString stringWithFormat:@"%@", _score.scoreValue];
    return cell;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    fb.completionHandler = ^(SLComposeViewControllerResult result){
        ...
        switch(result){
            ...
            case SLComposeViewControllerResultDone:{
                ...
                UIAlertView *nameAlert = [[UIAlertView alloc] initWithTitle:@"Share to Facebook" message:@"Shared to Facebook" delegate:nil cancelButtonTitle:@"Ok!" otherButtonTitles: nil];
                [nameAlert show];
    ...
    [fb setInitialText:[NSString stringWithFormat:@"Score Value of %@ While Playing Galactic Courier",  _score.scoreValue]];
    
    UIViewController *fbVC = self.view.window.rootViewController;
    [fbVC presentViewController:fb animated:YES completion:nil];
